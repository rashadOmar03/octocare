...
def test_purge_all_patients_removes_appointments(db_session):
    _seed_patient_with_appointment(db_session)
    stats = purge_all_patients(db_session)
    assert stats["patients_deleted"] == 1
...
def test_admin_purge_endpoint(db_session):
    ...
    with patch("routers.admin._notify_staff"):
        resp = client.post("/admin/purge-patients", json={"confirm": "DELETE_ALL_PATIENTS"})
    app.dependency_overrides.clear()
    assert resp.status_code == 200
    assert resp.json()["stats"]["patients_deleted"] == 1
